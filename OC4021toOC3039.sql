...
select
	...
	date_added,
	date_modified
from [opencart4_db].api_session a1
...
select *
from [opencart4_db].category_description cd1
where not exists (
	select 1
...
select *
from [opencart4_db].category_path cp1
where not exists
(
	select 1
...
-- select max(length(customer.password)) from [opencart4_db].customer;
-- select * from [opencart4_db].customer where length(customer.password) = 60;
...
-- Take salt from OC2.2.0.0 database
update [opencart3_db].customer coc3
set coc3.salt = (select coc.salt from belleson_cart.customer coc
where coc3.customer_id = coc.customer_id);

-- Take fax from OC2.2.0.0 database
update [opencart3_db].customer coc3
set coc3.fax = (select coc.fax from belleson_cart.customer coc
where coc3.customer_id = coc.customer_id);
...
select *
from [opencart4_db].product_to_category pc1
where not exists
...
select *
from [opencart4_db].product_to_store ps1
where not exists (
	select 1
...
select *
from [opencart4_db].product_attribute pa1
where not exists (
	select 1
...
select *
from [opencart4_db].product_option po
where not exists (
	select 1
...
select *
from [opencart4_db].product_option_value pov
where not exists (
	select 1
...
select *
from [opencart4_db].product_related pr1
where not exists (
...
,date_start
,date_end
from [opencart4_db].product_discount pd1
where not exists (
	select 1
...
-- select *
-- from [opencart4_db].product_image pi1
-- where not exists (
-- 	select 1
...
select product_id
	...
	,meta_description
	,meta_keyword
from [opencart4_db].product_description pd1
where not exists (
	select 1
...
-- select max(length(shipping_method)) from [opencart4_db].`order`;
-- select * from [opencart4_db].`order` where length(shipping_method) > 128;

-- Get rid of test orders entered in new system because shipping method is a long JSON text string
delete from [opencart4_db].`order` where length(shipping_method) > 128; -- order_id > 1704319966;
...
,`date_added`
,`date_modified`
from [opencart4_db].`order` o1 
where not exists (
	select 1 from [opencart3_db].`order` o2
	where o1.order_id = o2.order_id);

-- OC4 only
-- Update `order`.payment_address_id
-- update [opencart3_db].`order` ord2 join
-- (
-- 	select adr.address_id, ordr.order_id
-- 	from [opencart4_db].`order` ordr
-- 	join [opencart4_db].address adr on adr.customer_id = ordr.customer_id
-- 	where ordr.customer_id <> 0 and ordr.payment_address_id = -1
-- 	and ordr.payment_address_1 = adr.address_1
-- 	and (coalesce(ordr.payment_address_1, 'X') = coalesce(adr.address_2, 'X') or coalesce(ordr.payment_address_2, 'X') = coalesce(adr.address_2, 'X'))
-- 	and ordr.payment_city = adr.city
-- 	and coalesce(ordr.payment_postcode, '0') = coalesce(adr.postcode, '0')
-- ) as source
-- on ord2.order_id = source.order_id
-- set ord2.payment_address_id = source.address_id
-- where ord2.payment_address_id != source.address_id;

-- OC4 only
-- Update `order`.shipping_address_id using above subquery
-- update [opencart3_db].`order` ord2 join
-- (
-- 	select distinct ordr.order_id, adr.address_id -- , ordr.firstname, adr.firstname, ordr.shipping_address_1, adr.address_1, ordr.shipping_address_2, adr.address_2, ordr.shipping_city, adr.city, ordr.shipping_postcode, adr.postcode
-- 	from [opencart3_db].`order` ordr
-- 	join [opencart3_db].address adr on adr.customer_id = ordr.customer_id 
-- 	where ordr.customer_id <> 0 and ordr.shipping_address_id = -1
-- 	order by adr.customer_id
-- ) as source
-- on ord2.order_id = source.order_id
-- set ord2.shipping_address_id = source.address_id
-- where ord2.shipping_address_id != source.address_id;

-- OC4 only
-- Update `order`.payment_address_id using above subquery
-- update [opencart3_db].`order` ord2 join
-- (
-- 	select distinct ordr.order_id, adr.address_id -- , ordr.firstname, adr.firstname, ordr.payment_address_1, adr.address_1, ordr.payment_address_2, adr.address_2, ordr.payment_city, adr.city, ordr.payment_postcode, adr.postcode
-- 	from [opencart3_db].`order` ordr
-- 	join [opencart3_db].address adr on adr.customer_id = ordr.customer_id 
-- 	where ordr.customer_id <> 0 and ordr.payment_address_id = -1
-- 	order by adr.customer_id
...
,tax
,reward
from [opencart4_db].`order_product` op1
where not exists (
	select 1
...
select * from [opencart4_db].order_status os1
where not exists (
	select 1
...
select * from [opencart4_db].order_history oh1
where not exists (
	select 1
...
select * from [opencart4_db].order_option oo1
where not exists (
	select 1
...
,value
,sort_order
from [opencart4_db].order_total ot1
where not exists (
	select 1
...
select * from [opencart4_db].`return` r1
where not exists (
	select 1
...
select * from [opencart4_db].return_action ra1
where not exists (
	select 1
...
select * from [opencart4_db].return_history rh1
where not exists (
	select 1
...
select * from [opencart4_db].`return_reason` rr1
where not exists (
	select 1
...
select * from [opencart4_db].`return_status` rs1
where not exists (
	select 1
	from [opencart3_db].return_status rs2
	where rs1.return_status_id = rs2.return_status_id);

-- OC4 only, OC3 has different data set
-- insert into [opencart3_db].seo_url (
-- seo_url_id
-- ,store_id
-- ,language_id
-- ,`key`
...
-- ,keyword
-- ,sort_order
-- from [opencart4_db].seo_url su1
-- where not exists (
-- 	select 1
-- 	from [opencart3_db].seo_url su2
-- 	where su1.seo_url_id = su2.seo_url_id
-- 	and su1.store_id = su2.store_id
-- 	and su1.language_id = su2.language_id);

insert into [opencart3_db].`zone`
select * from [opencart4_db].`zone` z1
where not exists (
	select *
